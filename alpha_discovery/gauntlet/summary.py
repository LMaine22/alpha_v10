...
def _write_gauntlet_results(
    ...
    for result in stage_results:
        ...
        row = {
            ...
            # Clarify "pass" semantics: Deploy or Monitor are considered "passed_gauntlet"
            "passed_gauntlet": result["final_decision"] in ["Deploy", "Monitor"],
        }
    ...
    # Add descriptions from Pareto front if available, without duplicating rows
    try:
        pareto_df, _ = read_global_artifacts(os.path.dirname(gaunt_dir))
        if isinstance(pareto_df, pd.DataFrame) and not pareto_df.empty and "description" in pareto_df.columns:
            # Deduplicate by setup_id only; keep the first description observed
            pareto_desc = (
                pareto_df[["setup_id", "description"]]
                .dropna(subset=["setup_id"])
                .drop_duplicates(subset=["setup_id"], keep="first")
            )
            # Use a safe map to avoid row-multiplication on merge
            desc_map = dict(zip(pareto_desc["setup_id"], pareto_desc["description"]))
            results_df["description"] = results_df["setup_id"].map(desc_map)
    except Exception:
        pass

    if "description" not in results_df.columns or results_df["description"].isna().all():
        results_df["description"] = "Setup: " + results_df["signal_ids"].astype(str)

    # Ensure strictly one row per setup_id (guard against accidental duplication)
    results_df = results_df.drop_duplicates(subset=["setup_id"], keep="first")
...
def _write_open_positions(gaunt_dir: str, full_ledger: pd.DataFrame, stage_results: List[Dict[str, Any]]) -> str:
    ...
    # NEW LOGIC: Treat a setup as "open window" if its last trigger is within its trade horizon.
    # Even if trades are closed at end-of-data, we still surface setups whose signal window is active.
    if "trigger_date" not in ledger.columns or "horizon_days" not in ledger.columns:
        print("[Open Positions] Ledger missing trigger_date or horizon_days; writing empty file")
        empty_df = pd.DataFrame(columns=[
            "setup_id", "specialized_ticker", "direction", "entry_date",
            "days_open", "unrealized_pnl", "contracts", "strike", "option_type",
            "total_pnl", "nav_return_pct", "win_rate", "max_drawdown"
    ...
    # Determine reference "today" as the last available date in the ledger to avoid using wall-clock time
    ref_today = None
    for c in ("exit_date", "trigger_date"):
        if c in ledger.columns and pd.api.types.is_datetime64_any_dtype(ledger[c]):
            if ref_today is None or ledger[c].max() > ref_today:
                ref_today = ledger[c].max()
    if ref_today is None:
        ref_today = pd.Timestamp.now().normalize()
    else:
        ref_today = pd.to_datetime(ref_today).normalize()

    rows = []
    # Build one row per setup if its last trigger is within its horizon window
    for setup_id, g in ledger.groupby("setup_id"):
        if g.empty:
            continue
        # Find last trigger and corresponding horizon (max horizon on that date)
        g = g.copy()
        g["trigger_date"] = pd.to_datetime(g["trigger_date"], errors="coerce")
        g = g.dropna(subset=["trigger_date"])  # ensure datetimes
        if g.empty:
            continue
        last_trig = g["trigger_date"].max()
        last_day_rows = g[g["trigger_date"] == last_trig]
        try:
            last_h = int(pd.to_numeric(last_day_rows.get("horizon_days"), errors="coerce").max())
        except Exception:
            last_h = int(pd.to_numeric(g.get("horizon_days"), errors="coerce").median()) if "horizon_days" in g.columns else 0
        days_open = int((ref_today - last_trig.normalize()).days)
        if 0 <= days_open < max(1, last_h):
            r0 = last_day_rows.iloc[0]
            rows.append({
                "setup_id": setup_id,
                "specialized_ticker": r0.get("ticker", "Unknown"),
                "direction": r0.get("direction", "long"),
                "entry_date": last_trig,
                "days_open": days_open,
                # Economic fields are not meaningful without live mark; leave blank/NaN
                "unrealized_pnl": pd.NA,
                "contracts": r0.get("contracts", pd.NA),
                "strike": r0.get("strike", pd.NA),
                "option_type": r0.get("option_type", pd.NA),
            })

    if not rows:
        print("[Open Positions] No setups within their active horizon window; writing empty file")
        empty_df = pd.DataFrame(columns=[
            "setup_id", "specialized_ticker", "direction", "entry_date",
            "days_open", "unrealized_pnl", "contracts", "strike", "option_type",
            "total_pnl", "nav_return_pct", "win_rate", "max_drawdown"
