...
def run_gauntlet(
    ...
    candidates = pareto_summary.dropna(subset=["specialized_ticker", "direction", "signal_ids", "train_end_date"]).copy()
    ...
    for setup_id, oos_ledger in oos_ledgers.items():
        s1 = run_stage1_recency_liveness(
            run_dir=run_dir, fold_num=0, settings=settings, config=cfg,
        ...
        s2 = run_stage2_mbb_on_ledger(
            fold_ledger=oos_ledger, settings=settings, config=cfg, stage1_df=s1,
        )
    ...
    # Write diagnostics so you can audit exactly who failed Stage-1 and why
    if stage1_rows:
