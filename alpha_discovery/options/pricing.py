...
# ---- NEW: Enhanced IV Pricing System ----
...
def determine_strike_and_delta(
    ...
    """
    Determine strike K and deltas based on delta_bucket setting.
    ...
    if delta_bucket == 'AUTO_BY_DIRECTION':
        ...
        raise ValueError(f"Unknown delta_bucket: {delta_bucket}")
    
    # Solve for strike with improved error handling
    try:
    ...
    if K_solved is None:
        # Always fall back gracefully instead of raising errors
        K = S  # ATM fallback
        achieved_delta = bs_delta(S, K, T_years, r, sigma, option_type, q)
        return K, target_delta, achieved_delta
...
__all__ = [
    ...
    "get_anchor_iv", "map_anchor_to_tenor", "determine_strike_and_delta", "price_entry_exit_enhanced",
